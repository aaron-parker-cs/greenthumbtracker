...
export const validateHumidity = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  const { humidity, date, uomId } = req.body;
  const plantId = Number(req.params.plantId);

  if (!plantId || humidity === undefined) {
    res.status(400).json({
      message:
  ...
  if (isNaN(humidity)) {
    res.status(400).json({ message: "Humidity must be a number." });
    return;
  }
  ...
  if (date) {
    ...
    if (dateToCheck > now) {
      res.status(400).json({ message: "Humidity date cannot be in the future." });
      return;
    }
    if (dateToCheck < oneYearAgo) {
      res.status(400).json({
        message: "Humidity date cannot be more than 1 year in the past.",
      });
      return;
    }
  }
