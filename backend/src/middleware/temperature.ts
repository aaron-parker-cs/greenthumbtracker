...
export const validateTemperature = async (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> => {
  const { temperature, date, uomId } = req.body;
  const plantId = Number(req.params.plantId);

  if (!plantId || temperature === undefined) {
    res.status(400).json({
      message:
  ...
  if (date) {
    ...
    if (dateToCheck > now) {
      res.status(400).json({ message: "Temperature date cannot be in the future." });
      return;
    }
    if (dateToCheck < oneYearAgo) {
      res.status(400).json({
        message: "Temperature date cannot be more than 1 year in the past.",
      });
      return;
    }
  }
